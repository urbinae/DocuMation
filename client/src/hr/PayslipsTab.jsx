      ...
      const summary = {
        ...
        errors: (data.errors || []).map(err =>
          typeof err === 'object' && err !== null
            ? (err.sheet ? `[Hoja ${err.sheet}]: ` : '') + (err.error || JSON.stringify(err))
            : String(err)
        ),
        warnings: []
      };
                  ...
                  <>
                    ...
                    <div style={{ maxHeight: '120px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '11px', color: '#fde047' }}>
                      {uploadSummary.warnings.map((warn, idx) => {
                        const message = typeof warn === 'object' && warn !== null
                          ? `${warn.sheet ? `[Hoja: ${warn.sheet}] ` : ''}${warn.warning || warn.error || JSON.stringify(warn)}`
                          : String(warn);
                        return (
                          <div key={idx} style={{ wordBreak: 'break-all' }}>
                            ⚠️ {message}
